...
//     console.log("Received User ID:", req.body.user);
...
exports.resendOtp = async (req, res) => {
    try {
        ...
        if (!existingUser) {
            console.error("User not found for ID:", req.body.user);
            return res.status(404).json({ message: "User not found" });
        }
        ...
        const expirationTime = parseInt(process.env.OTP_EXPIRATION_TIME, 10);
        if (isNaN(expirationTime)) {
            console.error("Invalid OTP_EXPIRATION_TIME:", process.env.OTP_EXPIRATION_TIME);
            return res.status(500).json({ message: "Invalid OTP expiration time" });
        }
